...
public class ChangeStateTransitionCommandTests extends TestCase {
	public ChangeStateTransitionCommandTests(String name) {
		super(name);
	}
	
	public void test_change_add_state_transition()
	{
		String classId = "User";
		...
		addTransition.setInstanceId(instanceId);
		...
		EditorCommand_ChangeStateTransition changeTransition = new EditorCommand_ChangeStateTransition();
		changeTransition.setDomain(domain);
		changeTransition.setClassId(classId);
		changeTransition.setFromStateId(fromStateId);
		changeTransition.setToStateId(fromStateId);
		changeTransition.setInstanceId(instanceId);
		changeTransition.setSpecId(EntityClass.DEFAULT_EVENT_SPEC_ID);
		Assert.assertEquals(true, changeTransition.doCommand().returnStatus());

		Assert.assertEquals(fromState, fromState.getNextStateForEventSpecification(eventSpec));
