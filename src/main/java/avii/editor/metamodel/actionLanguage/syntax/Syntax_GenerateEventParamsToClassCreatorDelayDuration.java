...
public class Syntax_GenerateEventParamsToClassCreatorDelayDuration implements IGenerateActionLanguageSyntax, IGenerateDelayActionLanguageSyntax,
	...
	public String toString() {
		...
		output.append(") TO " + _Class + " CREATOR");
	...
	public boolean matchesLine(String line) {
		if (line.matches(regex)) {
			...
			_Class = m.group(m.groupCount() - 1);
			_Duration = m.group(m.groupCount());
			try {
				enumerateEventParameters(line, _eventName, _Class + " CREATOR DELAY " + _Duration);
			} catch (Exception e) {
				return false;
			}
			return true;
